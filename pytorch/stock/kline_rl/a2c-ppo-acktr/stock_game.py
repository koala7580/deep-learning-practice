...
class StockGame(gym.Env):
    ...
    def step(self, action):
        ...
        if not done:
            ...
            if self._hold_stock > 0:
                self._sell(d2['close'])
            reward = self._reward(d2['close'])
            # print('Done: %.2f' % reward)
            return self.observation, reward, done, { 'hold_stock': self._hold_stock, 'money': self._money }
    ...
    def _buy(self, price):
        ...
        if count > 0:
            ...
            # print('buy %.2f x %d, rest: %.2f' % (price, count, self._money))
...
　　     卖出后收入：101000元-101元-6元-101元=100792元；
        ...
        self._hold_stock = 0
        # print('sell %.2f x %d, rest: %.2f' % (price, n, self._money))
